...
class MainRecommender:
    ...
    def __init__(self, data: pd.DataFrame, weighting='bm25'):
        ...
        self.cos_recommender = self.fit_cosine_recommender(self.user_item_matrix)
    ...
    def fit_cosine_recommender(user_item_matrix):
        ...
        own_recommender = CosineRecommender(K=2)
        own_recommender.fit(csr_matrix(user_item_matrix).T.tocsr())
    ...
    def _get_recommendations(self, user, model, N=5, recalc_user=False):
    ...
    def get_cosine_recommendations(self, user, N=5):
        ...
        self._update_dict(user_id=user)
        return self._get_recommendations(user, model=self.own_recommender, N=N, recalc_user=True)
